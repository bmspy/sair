...
const DriverPlan = props => {
  ...
  useEffect(() => {
    ...
    const getPlanDetails = async () => {
      ...
      try {
        const response = await axios.get(`${API_URL}get/driver/month/plan/`, {
          params: {
            month: format(new Date(), 'MM', {locale: ar}),
            year: format(new Date(), 'yyyy', {locale: ar}),
          },
          headers: {
  ...
  const handleShare = () => {
    const planDetailsDate = {
      ...
      planDetailsMonth: format(new Date(), 'MM'),
    };
    let options = {
      html: HeadPlanHTML(planDetailsData, props.profile.full_name, 'سائق', planDetailsDate),
      fileName: 'plan_' + format(new Date(), 'MM') + '_' + format(new Date(), 'yyyy'),
      directory: 'Documents',
    };
          ...
          <ScrollView>
              ...
              <HStack style={{justifyContent: 'space-around'}}>
                <Text
                  ...
                  {/* {Object.keys(planDetailsData).length
                    ? format(
                        new Date(
                          `${props.planDetailsData?.year.toString()}-${`${props.planDetailsData?.month}`
                            .replace(/^(\d)$/, '0$1')
                            .toString()}`,
                        ),
                        'MMM',
            ...
            <VStack
              ...
              <ScrollView
                ...
                {!isLoading && planDetailsData?.plans?.length ? (
...
export default connect(mapStateToProps, mapDispatchToProps)(DriverPlan);
